...
> **Note:** The dataset is not included in this repository. Download it from the [official EdNet repository](https://github.com/riiid/ednet) and place the `KT3/`, `questions.csv`, and `lectures.csv` files under your configured input directory (see [Setup](#setup) below).
...
If running outside Kaggle, update `INPUT_DIR` and `OUTPUT_DIR` near the top of the notebook to point to your local dataset and desired output location.
...
📄 [`Adaptive_Personalized_Learning_Recommendation.pdf`](./Adaptive_Personalized_Learning_Recommendation.pdf)
